...
if not st.session_state.logged_in:
    ...
    if st.session_state.show_course_manager:
        ...
        with tab1:
            ...
            courses = load_courses()
            if courses:
                course_to_delete = st.selectbox("삭제할 강의 선택:", list(courses.keys()))
                if st.button("강의 삭제"):
        ...
        with tab2:
            ...
            if courses:
                selected_course = st.selectbox("강의 선택:", list(courses.keys()))
                ...
                if selected_course and selected_course in courses:
                    ...
                    if weeks:
                        ...
                        with col1:
                            edit_week = st.selectbox("수정할 주차:", weeks)
                        ...
                        edit_date = st.date_input("날짜 수정:", current_date)
                        
                        if st.button("주차 정보 수정"):
                            ...
                            if 'selected_image' in locals():
                                with st.spinner("OCR 수행 중..."):
                                    ...
                                    if result and result[0]:
                                        ...
                                        # 요약 옵션 추가
                                        if st.button("OCR 결과 요약하기"):
                                            with st.spinner("텍스트 요약 중..."):
                                                summary = summarize_text_with_zephyr(st.session_state.ocr_text)
                                                st.session_state.summary_text = summary
                                                st.text_area("요약 결과:", value=summary, height=150)
                            else:
